  ...
  return (
    <>
        ...
        <div className="container-page">
          ...
          <div className="mt-8 grid md:grid-cols-12 gap-8 items-end">
            <div className="md:col-span-8">
              ...
              <h1 className="mt-4 font-display font-light text-[clamp(2.5rem,6vw,5rem)] leading-[0.98] tracking-tightest text-ink">
                {project.title}
              </h1>
            </div>
            <p className="md:col-span-4 text-muted-foreground leading-relaxed">{project.summary}</p>
          </div>
        </div>
      </section>
      ...
      <section className="pt-16">
        <div className="container-page">
          <img src={cover} alt={project.title} className="w-full aspect-[21/10] object-cover" />
        </div>
      </section>

      <section className="py-24">
        <div className="container-page grid md:grid-cols-12 gap-12">
          <aside className="md:col-span-4 md:sticky md:top-28 h-fit space-y-8">
            <div>
              <div className="eyebrow text-accent">Service</div>
              <div className="mt-2 font-display text-xl text-ink">{project.service}</div>
            </div>
            <div>
              <div className="eyebrow text-accent">Timeline</div>
              <div className="mt-2 font-display text-xl text-ink">{project.timeline}</div>
            </div>
            <div>
          ...
          <div className="md:col-span-8 space-y-16">
            ...
            <div>
              ...
              <p className="mt-4 text-lg leading-relaxed text-foreground/85">{project.solution}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-ink text-background">
        <div className="container-page max-w-3xl">
          ...
          <blockquote className="mt-8 font-display font-light text-3xl md:text-4xl leading-snug tracking-tighter">
            &ldquo;{project.quote}&rdquo;
          </blockquote>
          <div className="mt-6 text-background/60">{project.quoteAuthor}</div>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {related.map((r: typeof projects[number]) => (
              <Link key={r.slug} to="/projects/$slug" params={{ slug: r.slug }} className="hover-image group">
                ...
                <h3 className="mt-4 font-display text-xl tracking-tighter text-ink">{r.title}</h3>
                <p className="text-sm text-muted-foreground">{r.suburb} · {r.year}</p>
              </Link>
            ))}
          </div>
        </div>
