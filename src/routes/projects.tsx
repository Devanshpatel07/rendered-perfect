...
function ProjectsList() {
  ...
  return (
    <>
      ...
      <section className="border-y border-line sticky top-20 bg-background/95 backdrop-blur z-30">
        <div className="container-page py-3.5 flex gap-2 overflow-x-auto no-scrollbar">
          {categories.map((c) => (
            <button
              key={c}
              onClick={() => setFilter(c)}
          ...
          <div className="grid grid-cols-12 gap-6 sm:gap-8 md:gap-10">
            {visible.map((p, i) => {
              ...
              return (
                ...
                >
                  <div className={`overflow-hidden bg-surface-2 ${wide ? "aspect-[4/3]" : "aspect-[4/3] sm:aspect-[4/5]"}`}>
                    <img src={covers[p.cover]} alt={p.title} className="h-full w-full object-cover" loading="lazy" />
                  </div>
                  <div className="mt-4 sm:mt-5 flex justify-between items-baseline gap-4">
                    <h3 className="font-display text-lg sm:text-xl md:text-2xl tracking-tighter text-ink">{p.title}</h3>
                    <span className="text-xs sm:text-sm text-muted-foreground shrink-0">{p.suburb} · {p.year}</span>
                  </div>
                  <p className="text-xs sm:text-sm text-muted-foreground mt-1">{p.service} · {p.category}</p>
                </Link>
              );
            })}
          </div>
