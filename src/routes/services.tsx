          ...
          <div className="border-t border-line">
            {services.map((s) => (
              <Link
                ...
                to="/services/$slug"
                params={{ slug: s.slug }}
                className="group grid grid-cols-1 md:grid-cols-12 items-baseline gap-3 sm:gap-6 py-6 sm:py-10 border-b border-line hover:bg-surface/60 transition-colors -mx-2 px-3 sm:px-2"
              >
                <div className="flex items-center justify-between md:col-span-1">
                  <span className="font-mono text-[11px] text-accent font-semibold">{s.index}</span>
                  <span className="md:hidden eyebrow text-accent">→</span>
                </div>
                <div className="md:col-span-4">
                  <h2 className="font-display text-2xl sm:text-3xl md:text-4xl tracking-tightest text-ink group-hover:text-accent transition-colors">
                    {s.title}
                  </h2>
                </div>
                <p className="md:col-span-6 text-sm sm:text-base text-muted-foreground leading-relaxed">{s.short}</p>
                <span className="hidden md:block md:col-span-1 justify-self-end eyebrow text-ink group-hover:text-accent transition-colors">→</span>
              </Link>
            ))}
          </div>
        </div>
