...
function ServiceDetail() {
  ...
  return (
    <>
      <section className="relative pt-32 pb-16 md:pt-52 md:pb-32">
        <div className="container-page grid grid-cols-1 md:grid-cols-12 gap-8 sm:gap-12 items-end">
          <div className="md:col-span-7">
            <Link to="/services" className="eyebrow text-accent link-underline">← All services</Link>
            <div className="mt-6 sm:mt-8 font-mono text-sm text-muted-foreground">{service.index}</div>
            <h1 className="mt-3 sm:mt-4 font-display font-light text-[clamp(2.2rem,6vw,5.5rem)] leading-[0.98] tracking-tightest text-ink">
              {service.title}
            </h1>
            <p className="mt-4 sm:mt-8 text-base sm:text-lg text-muted-foreground max-w-xl leading-relaxed">{service.short}</p>
          </div>
          <div className="md:col-span-5">
            <img src={heroFacade} alt={service.title} className="w-full aspect-[16/10] sm:aspect-[4/5] object-cover" loading="lazy" />
          </div>
        </div>
      </section>

      <section className="py-12 sm:py-20 bg-surface">
        <div className="container-page grid grid-cols-1 md:grid-cols-12 gap-6 sm:gap-12">
          <div className="md:col-span-5">
            <EyebrowHeading eyebrow="Overview" title={<>What it is, and why.</>} />
          </div>
          <div className="md:col-span-7">
            <p className="text-base sm:text-lg text-foreground/85 leading-relaxed">{service.overview}</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 sm:py-20">
        <div className="container-page grid grid-cols-1 md:grid-cols-12 gap-8 sm:gap-12">
          <div className="md:col-span-6">
            ...
            <ul className="mt-6 sm:mt-8 divide-y divide-line border-y border-line">
              {service.benefits.map((b: string) => (
                <li key={b} className="flex items-baseline gap-4 py-3 sm:py-4">
                  <span className="text-accent font-mono text-xs">◆</span>
                  <span className="text-sm sm:text-base text-ink">{b}</span>
                </li>
              ))}
            </ul>
          </div>
          <div className="md:col-span-6">
            ...
            <ul className="mt-6 sm:mt-8 divide-y divide-line border-y border-line">
              {service.applications.map((a: string) => (
                <li key={a} className="flex items-baseline gap-4 py-3 sm:py-4">
                  <span className="text-accent font-mono text-xs">◇</span>
                  <span className="text-sm sm:text-base text-ink">{a}</span>
                </li>
              ))}
            </ul>
          </div>
      ...
      <section className="py-12 sm:py-20">
        <div className="container-page">
          ...
          <div className="mt-8 sm:mt-14 grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
            {gallery.map((src, i) => (
          ...
          <div className="md:col-span-8">
            <ol className="space-y-4 sm:space-y-6">
              {["Consultation", "Preparation", "Application", "Handover"].map((step, i) => (
                <li key={step} className="flex gap-4 sm:gap-6 border-t border-line pt-4 sm:pt-5">
                  <span className="font-mono text-accent text-xs">{String(i + 1).padStart(2, "0")}</span>
                  <span className="font-display text-lg sm:text-xl tracking-tighter text-ink">{step}</span>
                </li>
              ))}
            </ol>
          </div>
        ...
        <div className="container-page grid grid-cols-1 md:grid-cols-12 gap-8 sm:gap-12">
          ...
          <div className="md:col-span-8">
            {faqs.slice(0, 4).map((f) => (
              <details key={f.q} className="group border-t border-line py-5 sm:py-6 last:border-b">
                <summary className="flex justify-between gap-4 sm:gap-6 cursor-pointer list-none">
                  <span className="font-display text-lg sm:text-xl tracking-tighter text-ink">{f.q}</span>
                  <span className="font-mono text-accent transition-transform group-open:rotate-45">+</span>
                </summary>
                <p className="mt-2 sm:mt-3 text-sm sm:text-base text-muted-foreground leading-relaxed max-w-2xl">{f.a}</p>
              </details>
            ))}
          </div>
        </div>
      ...
      <section className="bg-ink text-white py-16 sm:py-24">
        <div className="container-page text-center max-w-2xl mx-auto">
          ...
          <Link to="/contact" className="mt-6 sm:mt-8 inline-flex bg-accent text-accent-foreground px-8 py-5 text-[11px] font-semibold uppercase tracking-[0.2em] hover:bg-background hover:text-ink transition-colors">
            Request a quote
          </Link>
        </div>
      </section>
